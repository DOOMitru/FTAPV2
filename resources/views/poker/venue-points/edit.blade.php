<x-app-layout>
    ...
    <div class="l-container">
        <x-card>
            <form method="POST" action="{{ route('poker.venue-points.update', $venue_point) }}" class="l-stack">
                @csrf
        @method('PATCH')
                    ...
                    <select class="field__control" name="venue_id" id="venue_id" required>
                        ...
                        @foreach ($venues as $venue)
                            <option value="{{ $venue->id }}"
                                    @selected(old('venue_id', $venue_point->venue_id) == $venue->id)>{{ $venue->name }}</option>
                        @endforeach
                    </select>
                </x-field>
                    ...
                    <select class="field__control" name="user_id" id="user_id" required data-autofill='{"first":"#user_name"}'>
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" data-first-name="{{ $user->first_name }}"
                                    ...
                                    @selected(old('user_id', $venue_point->user_id) == $user->id)>{{ $user->first_name.' '.$user->last_name }}</option>
                        @endforeach
                    </select>
                </x-field>

                <x-field name="user_name" :label="__('Player Name')" :value="old('user_name', $venue_point->user_name)" required />
                ...
                <div class="l-cluster">
                    ...
                    <a class="link" href="{{ route('poker.venue-points.index') }}">{{ __('Cancel') }}</a>
                </div>
            </form>
        </x-card>
    </div>
