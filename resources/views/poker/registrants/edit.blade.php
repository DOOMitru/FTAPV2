        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('poker.registrants.update', $registrant) }}" class="space-y-6">
                        @csrf
                        @method('PATCH')
                        ...
                        <div>
                            ...
                            <select id="tournament_id" name="tournament_id" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required autofocus>
                                @foreach ($tournaments as $tournament)
                                    <option value="{{ $tournament->id }}" {{ old('tournament_id', $registrant->tournament_id) == $tournament->id ? 'selected' : '' }}>{{ $tournament->name }} ({{ $tournament->start_time }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input id="registered_at" name="registered_at" type="datetime-local" class="mt-1 block w-full" :value="old('registered_at', \Illuminate\Support\Carbon::parse($registrant->registered_at)->format('Y-m-d\TH:i'))" required />
                                ...
                                <select id="user_id" name="user_id" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}" 
                                            {{ old('user_id', $registrant->user_id) == $user->id ? 'selected' : '' }}
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input id="player_name" name="player_name" type="text" class="mt-1 block w-full" :value="old('player_name', $registrant->player_name)" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="player_nickname" name="player_nickname" type="text" class="mt-1 block w-full" :value="old('player_nickname', $registrant->player_nickname)" />
                                <x-input-error class="mt-2" :messages="$errors->get('player_nickname')" />
                            </div>
                        </div>

                        <div class="flex items-center gap-4">
                            <x-primary-button>{{ __('Update Registrant') }}</x-primary-button>
